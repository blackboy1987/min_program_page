...
public abstract class BaseDaoImpl<T extends BaseEntity<ID>, ID extends Serializable> implements BaseDao<T, ID> {
	...
	protected Long count(CriteriaQuery<T> criteriaQuery1, List<Filter> filters) {
		...
		Predicate newRestrictions = buildPredicate(criteriaBuilder, criteriaQuery1.getRestriction());


		Predicate restrictions = newRestrictions != null ? newRestrictions : criteriaBuilder.conjunction();
		restrictions = criteriaBuilder.and(restrictions, toPredicate(root, filters));
		criteriaQuery.where(restrictions);
	...
	private Predicate buildPredicate(CriteriaBuilder criteriaBuilder, Predicate restriction) {
		if(restriction!=null){
			Predicate restrictions = criteriaBuilder.conjunction();
			List<Expression<Boolean>> expressions = restriction.getExpressions();
			for (Expression<Boolean> expression : expressions) {
				restrictions = criteriaBuilder.and(restrictions, expression);
			}
			return restrictions;
		}
		return null;
